...
ssh "$VPS_HOST" \
  "BACKUP_FILE='$BACKUP_FILE' CONTAINER_NAME='$CONTAINER_NAME' POSTGRES_IMAGE='$POSTGRES_IMAGE' POSTGRES_PASSWORD='$POSTGRES_PASSWORD' RESTORE_ROLE='$RESTORE_ROLE' RESTORE_DB='$RESTORE_DB' bash -s" <<'REMOTE'
set -euo pipefail
