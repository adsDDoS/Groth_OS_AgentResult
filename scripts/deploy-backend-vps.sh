...
ssh "$VPS_HOST" \
  "APP_DIR='$APP_DIR' CONTAINER_NAME='$CONTAINER_NAME' NETWORK_NAME='$NETWORK_NAME' RUNTIME_DIR='$RUNTIME_DIR' HOST_BIND='$HOST_BIND' HOST_PORT='$HOST_PORT' CONTAINER_PORT='$CONTAINER_PORT' ALLOW_LOCAL_STORAGE='$ALLOW_LOCAL_STORAGE' bash -s" <<'REMOTE'
set -euo pipefail
