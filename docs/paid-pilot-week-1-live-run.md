...
| --- | --- | --- | --- | --- | --- |
| Onboarding | Telegram `/onboarding` / dashboard Start Pilot | Intake completed. | Live Telegram onboarding completed; messages delivered in owner DM. | "Hermes agent в телеграм должен присылать ссылки для проверки в личку, также должно быть в дашборде где-то грамотно выполнено." | Backend `/telegram/commands/send` delivery was initially not configured, so operator used backend command + direct Telegram send workaround. Fixed in `f8f4d3e`: delivery chat falls back to allowed owner user and actual `/telegram/commands/send` sent `messageId: 310`. |
| First material | Telegram advisor + dashboard board | One material brief ready for owner decision. | Hermes task initially failed with `fetch failed`; fallback draft created through backend material command: content `2e8ef890-560c-4a09-864f-88a18e699e13`, approval `baf9bd17-d1e7-4d3e-8d2b-88d8369e9d3d`, Telegram notification `messageId: 306`, full draft DM `messageId: 309`. Stale demo approval `10000000-0000-4000-8000-000000000403` was rejected through product API so owner-control shows only the real material. Production Hermes dispatch was then restored: integrated smoke task `9d96863c-30c1-435c-95ab-241842ff1509` completed, draft `1513f563-0933-49b5-ab6d-ec30ae8258cf` was created, and smoke approval `663ce881-23d1-496f-8afe-6c55075c57b9` was rejected for cleanup. |  | Owner approval loop can continue from the real first draft; future `/prepare` runs no longer require operator fallback. |
...
| --- | --- | --- | --- | --- | --- | --- |
| F-001 | Onboarding | P2 | Owner expects Telegram direct links and a clear dashboard `done` state. | Result confirmation UX may still be too backend-shaped for a real owner. | Dashboard copy now uses owner-facing `Выполнено`; URL confirmation remains a required live check in week 1. | Verify real publication URL confirmation after the first owner-approved release. |
...
| F-003 | First material | fixed | Hermes onboarding first-material task failed with `fetch failed`; after sidecar start, dispatch failed again because Hermes profile used unaffordable `anthropic/claude-opus-4.6` and requested 128k tokens. | `agentresult-os-hermes` was stopped; backend did not guard Hermes reachability; Hermes API profile was too expensive for production smoke. | Started `agentresult-os-hermes`, added health guard, switched sidecar profile to `openrouter/free`, capped backend Hermes API request at `HERMES_MAX_TOKENS=2048`, and verified integrated `/telegram/commands/send prepare ...` completion. | Keep `agentresult-os-hermes` running for paid pilot and add a non-mutating Hermes dispatch canary later. |
