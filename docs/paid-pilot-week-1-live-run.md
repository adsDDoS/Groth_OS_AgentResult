...
- Owner Telegram: pending live `/onboarding` in owner-control.
- Operator: AgentResult operator.
- Approval owner: owner.
- QA/release owner: owner.
- Result owner: Hermes agent + owner dashboard confirmation.
...
- Start date: 2026-06-23 intake captured; live `/onboarding` pending.
- Day-7 review date:
...
| Owner Telegram access confirmed. | pending | Must be confirmed by live `/onboarding` in Telegram owner-control. |
...
| `/onboarding` starts and shows `Шаг 1/7`. | pending | Run in Telegram owner-control with the script below. |
...
| --- | --- | --- | --- | --- | --- |
| Onboarding | Telegram `/onboarding` / dashboard Start Pilot | Intake completed. | Intake captured in docs; live Telegram `/onboarding` pending. | "Hermes agent в телеграм должен присылать ссылки для проверки в личку, также должно быть в дашборде где-то грамотно выполнено." | Need verify Telegram links + dashboard completed state during real run. |
