...
`GET /pilot/week-2/execution` returns the active backend-owned week-2 working surface for dashboard and Telegram. It reads the active pilot workspace, next material, `social_post` material approval, week-2 board, roles, publication result, and allowed action targets. It does not create or mutate domain objects.
...
{
  "status": "active",
  "current_gate": "material_approval",
  "material": {},
  "material_approval": {},
  "board": [],
  "task": {},
  ...
  "roles": {
    ...
    "release_owner": "Content operator or chief of staff",
    "result_owner": "Content operator or chief of staff"
  },
  "actions": {
    "approve_material": { "approval_id": "..." },
    "handoff_release": { "calendar_item_id": "..." },
    "confirm_url": { "calendar_item_id": "..." },
    "review_result": { "publication_result_id": "..." }
  }
}
```
...
`POST /pilot/week-4/start` starts backend-owned week-4 execution only after the `pilot_week_4_scope` approval is approved. It uses the same generic week execution command path as week 2 and week 3: the next material moves into review, the first week-4 board item is marked started, a `pilot_week_4_execution` task is created, a week-4 material approval is opened, tenant workspace state moves to `mode: "week_4"`, and owner-action audit records `pilot.week_4.start`. Repeating the command is idempotent and returns the existing execution objects when week-4 execution is already started or completed.
