  ...
  content_items: [
    {
      ...
      title: "Почему контент-ферме нужен операционный контур, а не один AI-агент",
      content_type: "telegram_post",
      channel: "telegram",
      status: "review",
      target_url: "https://t.me/agentresult",
      body_md:
        "Один AI-агент не делает контент-ферму управляемой.\n\nЕсли нет контура производства, он просто добавляет ещё один поток черновиков.\n\nЧто должно быть вместо этого:\n- темы фиксируются и не теряются\n- тексты доходят до QA\n- материалы проходят согласование до выхода\n- публикация подтверждается URL и первичной реакцией\n- следующий контент-шаг возвращается в систему\n\nAgentResult OS строится как такой контур: AgentResult готовит, собственник принимает решение, выпуск фиксируется, результат возвращается в систему.\n\nСледующий шаг — согласовать первый материал и проверить, какой сигнал он даст.",
      metadata: { owner: "Egor", idea: "content-ops", module: "Content Factory" },
      created_at: now,
      updated_at: now
    },
    {
